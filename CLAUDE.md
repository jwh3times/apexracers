...
- `AccountEmailTemplates` — pure builders for the password-reset, email-change-verification, and email-change-notice (security alert to the old address; HTML-encodes the requested address) emails (branded HTML + plain-text alternate, CTA + raw-link fallback); unit-tested directly (mirrors `AchievementsMapper`)
...
`ApexRacers.Seeder --demo` runs a `DemoCacheSeeder` step (in addition to the standard catalog + synthetic subsession seed) that seeds `ExternalDataCaches` rows with synthetic mapped DTOs under each service's exact cache keys — the same keys that `CachedIRacingClient` writes at runtime — marked with a far-future `ExpiresAt` sentinel (`>= 9000-01-01`) so the cleanup service never evicts them. It also seeds `SeasonCarBops` (synthetic BoP) and `Weeks.WeatherSummaryJson` (synthetic weather) for active seasons (Strategy/Schedule), the percentile **world-record** overlay (`wr:{carId}:{trackId}` = field-best × 0.98 — `WorldRecordService` reads a cached `wr:` row even when creds are absent), Race Detail **lap traces** (`laps:{subsessionId}:{demo driver}`, stats via the real `LapAnalysis.Compute`), and a curated set of `/compare` **driver-search** terms (`driversearch:{term}`). The Seeder project references `ApexRacers.Api` to reuse the real cached DTO types — the cache-row records (`ProfileSnapshot`/`LicenseSnapshot`/`RecapSnapshot`, `RecentRaceCacheRow`, `RaceGuideCacheRow`) were promoted from `private` nested to `public` top-level so the seeder can build them — keeping the seeded JSON identical in shape to what live services write.
...
**Known demo caveats** (not page-breakers): `/analytics` populates lazily after a Recommendations/percentile visit; the race-guide board shows static "in-progress" sessions (a sentinel-cached row can't track the clock); and the `/compare` driver-search box only returns hits for a curated term set (e.g. "rival", "demo", "driver") — **arbitrary** unseeded terms 503, so use the shared-race **suggestions** list to add an unlisted rival.
