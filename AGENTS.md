...
than as throttling. Set any of the three vars in `.env` to exercise the limiter locally. This per-IP
`auth` policy is unauthenticated-request-volume protection and is independent of the sign-in throttle
(`SignInThrottleStore`, see Services) that counts failed passwords per (account, source address) —
the two are separate layers, not one restated as the other. Health probes (anonymous, rate-limit-exempt): `GET /healthz` (liveness, no
...
comparison and keeping only the number. A compact response that omits the lap time but presents a
metric derived from that Personal Best still carries its evidence; Week Detail's `my-percentiles`
response is the canonical example. `CONTEXT.md`'s Lap Evidence section defines Race Lap/Uploaded
...
Track Configuration Name absence is a fourth shared boundary rule:
`ApexRacers.Core.ConfigurationName.Normalize` collapses omitted, blank, and `N/A` inputs to the empty
string before any `Track.ConfigName` persistence, while `NullIfAbsent` converts that storage value to
...
Sign-in brute-force refusal is a sixth shared boundary rule: `ApexRacers.Core.SignInThrottle` (+
`SignInThrottleOptions.Validated()`) decides, from failure counts and a clock alone, whether one more
sign-in attempt from one source address is allowed — scoped to (account, source address) rather than
to the account alone, which is what let a stranger who merely knew a Driver's email lock that Driver
out indefinitely (issue #300, closing the last open finding of GHSA-28pc-cx5w-g6jp). Crossing a much
higher account-wide failure count never denies a caller by itself; it only tightens every address's
allowance (5 down to 1), so an address with no failures against it is still admitted on the first
correct password. `Validated()` fails startup if the tightened allowance or the high-water mark could
be 0, since either would silently deny the account owner too. `SignInThrottleStore` is the persistence
half (`identity.SignInAddressFailures` / `identity.SignInAccountFailures`); `dotnet-api` carries the
call rules and `postgres-specialist` the schema.
...
- `PercentileCalculationService` — compute + cache percentile (rank + median and presentability via `Core.FieldPercentile`; best lap + evidence via `Core.PersonalBest.Select`, with the Uploaded side bounded to the Race Week via `Core.RaceWeekWindow`); overlays world-record via `WorldRecordService`. Also reports the fastest Uploaded Lap the bound excluded (`UploadedBestOutsideWeekDto`) when it was faster than the Personal Best that got ranked.
  ...
  Customer ID, preserving the eligible Demo Driver path. Other Driver lookups remain readable without
  a cache write. Uploaded Lap access still requires the caller's own Claimed Identity; demo resolution
  does not grant access to another User's Uploaded Laps.
- `CarRecommendationService` — ranked recommendations from personal percentile data (rank and presentability via `Core.FieldPercentile`; best lap + evidence via `Core.PersonalBest.Select`, Uploaded side bounded to the Race Week via `Core.RaceWeekWindow`). Its contract keeps a current-week `PercentileRank` separate from the historical `ExpectedPercentile` used for projection; a projected-only car has no Percentile Rank, Top Share, or Field Size because the Driver did not enter that Field. Undersized Field readings remain cached with their Field Size but do not enter the running Expected Percentile or Week Detail's `my-percentiles` overlay; each returned Week Detail reading carries `PersonalBestLapEvidence` even though the compact DTO omits the lap time itself.
...
- `UserAnalyticsService` — per-car percentile history/stats (median and minimum presentable Field Size via `Core.FieldPercentile`; per-week best lap + evidence via `Core.PersonalBest.Select`, kept per (car, week) so the fastest lap across counted weeks carries its own provenance). Undersized Field readings are excluded. Each result row's Uploaded Laps are bounded to that row's own Race Week via `Core.RaceWeekWindow`, resolved per season since a season's Weeks can each need a different fallback boundary.
...
- `RefreshTokenStore` — owns issue/rotate/revoke/revoke-all/retention cleanup over an injected `TimeProvider`. Its canonical active predicate is `RevokedAt == null && ExpiresAt > now`; issuance caps active tokens at 5 per user. Rotation consumes the presented token with a conditional update rather than a plain read-then-write, so two requests racing the same token can never both succeed, then inserts the replacement in the same transaction; losing that race is a generic rejection, never a reuse-triggered revocation. `RevokeAllActiveAsync` re-sweeps until a pass finds nothing left active, so a token issued mid-revocation is still caught. Raw tokens leave only as return values; persistence stores their SHA-256 hashes.
- `SignInThrottleStore` — the persistence half of the sixth shared boundary rule above: reads and records the failure counters `Core.SignInThrottle` decides on. `ClaimAttemptAsync` claims one attempt with a single atomic upsert **before** the password is checked, so a concurrent burst against one address gets distinct counts rather than every request reading the same stale one and all passing the gate; a refused attempt records nothing, so a caller can't hold its own window open by continuing to knock. `NoteFailureAsync` paces the owner's "someone is guessing" notice to one per account per `SignInThrottleOptions.NoticeInterval`. `SignInThrottleCleanupService` purges rows once their window plus a grace period has passed.
