...
- `CarRecommendationService` — ranked recommendations from personal percentile data (rank and presentability via `Core.FieldPercentile`; best lap + evidence via `Core.PersonalBest.Select`, Uploaded side bounded to the Race Week via `Core.RaceWeekWindow`). Undersized Field readings remain cached with their Field Size but do not enter the running Expected Percentile or Week Detail's `my-percentiles` overlay.
