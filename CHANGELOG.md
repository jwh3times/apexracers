...
- ApexRacers now shows its own icon everywhere a browser or device displays one — the browser
  tab, a bookmark, and an iOS or Android home-screen shortcut. The mark ships as a scalable
  SVG, a multi-resolution `.ico`, and PNGs from 16x16 to 1024x1024, in both a
  transparent and a dark-background variant.

### Fixed

- The browser tab showed a blank or default icon instead of the app’s. The page requested an
  icon file that was no longer present, so browsers had nothing to draw.
...
- Reject malformed `.ibt` data-buffer offsets and sizes that exceed the file bounds before seeking
  or allocating the record buffer.
...
- The admin role-management endpoint now rejects promotion to Admin as well as changes to existing
  Admin memberships. Rejected requests preserve the account's current role; startup seeding remains
...
- Startup admin seeding now requires a confirmed email before promoting a configured account.
  Unconfirmed accounts retain their current roles; existing Admin memberships are preserved.
  Fresh admin bootstrap requires operator verification and confirmation before restarting the API.
