...
// Public — the track catalog is non-sensitive reference data. When a valid token is present the
// "your best laps" overlay on detail is populated for that user; otherwise it is omitted.
[ApiController]
[Route("api/tracks")]
public class TracksController(TrackCatalogService catalog) : ControllerBase
{
    [HttpGet]
