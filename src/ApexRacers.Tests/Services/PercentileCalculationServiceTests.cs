...
public class PercentileCalculationServiceTests
{
    ...
    public async Task ComputeAndCacheAsync_DriverHasNoLapTime_ReturnsNull()
    {
        ...
        db.LapTimeEntries.Add(new LapTimeEntry { WeekId = 10, CarId = 1, DriverCustomerId = 999, LapTimeSeconds = 70, Car = car, Week = week, RecordedAt = DateTimeOffset.UtcNow });
        await db.SaveChangesAsync();

        var result = await new PercentileCalculationService(db).ComputeAndCacheAsync(seriesId: 1, weekId: 10, carId: 1, customerId: 1);
    ...
    public async Task ComputeAndCacheAsync_DriverBeats3Of4Others_Returns75Percentile()
    {
        await using var db = DbContextFactory.Create();
        var (week, car) = SeedWeekAndCar(db);
        // Driver has the 2nd fastest time; beats 3 of the other 4 drivers.
        // slowerCount=3, total=5, percentile = 3*100/(5-1) = 75
        db.LapTimeEntries.AddRange(
            new LapTimeEntry { WeekId = 10, CarId = 1, DriverCustomerId = 1, LapTimeSeconds = 65, Car = car, Week = week, RecordedAt = DateTimeOffset.UtcNow },
            new LapTimeEntry { WeekId = 10, CarId = 1, DriverCustomerId = 2, LapTimeSeconds = 60, Car = car, Week = week, RecordedAt = DateTimeOffset.UtcNow },
            new LapTimeEntry { WeekId = 10, CarId = 1, DriverCustomerId = 3, LapTimeSeconds = 70, Car = car, Week = week, RecordedAt = DateTimeOffset.UtcNow },
            new LapTimeEntry { WeekId = 10, CarId = 1, DriverCustomerId = 4, LapTimeSeconds = 80, Car = car, Week = week, RecordedAt = DateTimeOffset.UtcNow },
            new LapTimeEntry { WeekId = 10, CarId = 1, DriverCustomerId = 5, LapTimeSeconds = 90, Car = car, Week = week, RecordedAt = DateTimeOffset.UtcNow });
        await db.SaveChangesAsync();
    ...
    public async Task ComputeAndCacheAsync_UserProfileExists_CreatesCarPercentileResult()
    {
        ...
        await new PercentileCalculationService(db).ComputeAndCacheAsync(seriesId: 1, weekId: 10, carId: 1, customerId: 1);
