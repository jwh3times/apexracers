...
- Flag a caller's `scope` argument to `UploadedBestQuery.RunAsync` that already filters on `IsValidLap` — harmless (the module re-applies its own filter), but a sign the caller doesn't know the module owns that invariant.
