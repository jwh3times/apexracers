...
- Flag any `LoginAsync`/`AuthController` change that reintroduces a status, body, or return shape
  that varies with *why* a sign-in failed (a `LoginResult`-shaped type, a distinct lockout status
  code, or any field beyond the single `AuthResultDto?`) — that pairing is GHSA-28pc-cx5w-g6jp: only
  a real account can be locked, so a status unique to lockout enumerates the user list.
- Flag a lockout ever reported in the sign-in response itself, including on a *correct* password —
  that turns the lockout window into a password oracle. The lockout may only be disclosed by email
  (`AccountEmailTemplates.AccountLocked`), sent once per lockout, never once per attempt.
- Flag `AccessFailedAsync` called from the already-locked-out branch — Identity restarts the lockout
  window on every recorded failure, so counting attempts made during a lockout lets a stranger hold
