...
| `Subsessions`          | int PK      | One Split of a Race Session (RaceSessionId, SeasonId, RaceWeekIndex, WeekId, TrackId, OfficialSession, EventStrengthOfField, StartTime, SplitIndex, SplitCount, TeamEntryCount, AiEntryCount); nullable indexed `RaceSessionId` persists iRacing's `session_id` so sibling Splits can be grouped, while null identifies a row ingested before it was persisted. `WeatherJson` / `TrackStateJson` serialize owned `WeatherSnapshot` / `TrackStateSnapshot` contracts. `SplitIndex` / `SplitCount` are nullable and derived at ingest by ranking the payload's Splits on Strength of Field, not iRacing fields; null is an unknown position and must never be read as index 0. `TeamEntryCount` / `AiEntryCount` are likewise nullable — they count the payload entries that produced no `SubsessionResult` row, so null means never counted rather than a complete field |
...
| `SeasonCarBops`        | composite   | Per-week BoP for one car (SeasonId, RaceWeekIndex, CarId); `CarId` has no FK (BoP ingestion order never blocks on catalog)                                      |
...
`SeasonCarBop` has an index on `(SeasonId, RaceWeekIndex)` — backs the per-week BoP lookup from `ScheduleService`/`StrategyService`.
