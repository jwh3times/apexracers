...
| `Subsessions`          | int PK      | One Split of a Race Session (SeasonId, WeekNumber, WeekId, TrackId, OfficialSession, EventStrengthOfField, StartTime, SplitIndex, SplitCount); `WeatherJson` / `TrackStateJson` serialize owned `WeatherSnapshot` / `TrackStateSnapshot` contracts. `SplitIndex` / `SplitCount` are nullable and derived at ingest by ranking the payload's Splits on Strength of Field, not iRacing fields; null is an unknown position and must never be read as index 0. iRacing's session identity is not stored, so Splits of one Race Session cannot be reassociated |
