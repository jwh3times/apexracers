...
- Run `node scripts/sync-agent-configs.mjs` (or `npm run sync:agents`) and commit the regenerated
  ...
  the **Agent Config Sync** CI check reports a failure on drift (it isn't a required status check
  today, but treat a red run as blocking anyway). `.claude/hooks/*` is tool-specific and is not
  mirrored into `.codex/`.
