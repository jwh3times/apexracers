---
name: docs-updater
description: Use to keep public documentation and an installed private companion current after code changes — AGENTS.md (canonical), README.md, private/PRD.md, and the agent sources in .claude/agents/. Run after completing a feature, security fix, or architectural change.
...
generated from it by `node scripts/sync-agents.mjs`. The `.agents/skills/*/**` block runs the
...
Paths under `private/` belong to an optional standalone repository checked out inside the public
worktree. Require `private/.git` before editing them, inspect their diff from that root, and report
their commit/push state separately. If the companion is absent, keep public documentation work
functional and report which private reconciliation could not be performed.
...
- Installed private deployment runbook: exact resource names, secret names, and provisioning commands
...
- Run `node scripts/sync-agents.mjs` (or `npm run sync:agents`) and commit the regenerated
  ...
  the **Agent Config Sync** CI check fails the PR on drift — it is a required status check, so the
  PR cannot merge until the regenerated output is committed. `.claude/hooks/*` is tool-specific and
  is not mirrored into `.codex/`.
...
**Outer Git history is never evidence about the nested companion.** When `private/.git` exists, use
`git -C private status`, `git -C private log`, and the file content itself. Compare the newest archive
headings against recently merged public work; a missing entry means the archive is behind. When the
companion is absent, report the unavailable check instead of creating `private/` or inferring that no
update is needed.
...
- Do not create private documents unless `private/.git` confirms the companion repository is installed.
