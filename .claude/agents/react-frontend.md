...
- Every `vi.fn()` must supply its type parameters (`vi.fn<(arg: Foo) => Bar>()`, not a bare `vi.fn()`) — Oxlint's `vitest/require-mock-type-parameters` is an `error` in `web/.oxlintrc.json`, not a warning, so an untyped mock fails the `Lint` CI job. It was promoted to `error` deliberately rather than turning on `--deny-warnings` repo-wide: Dependabot bumps `oxlint` daily in a catch-all group, and a new warn-level rule appearing in that bump would otherwise redden CI on unrelated dependency PRs (`react/exhaustive-deps` stays `warn` for the same reason).
- `src/test/jest-dom-matchers.d.ts` restores `@testing-library/jest-dom`'s matcher types (`toBeInTheDocument`, etc.) under Vitest 5: Vitest 5 inlined `expect` with a two-type-parameter `Assertion<R, T>`, and jest-dom's own one-parameter augmentation (`types/vitest.d.ts`) silently stops merging against it, so `tsc`/`npm run build` fails while the tests themselves stay green. The shim augments `Matchers` instead (the extension point `Assertion`/`AsymmetricMatchersContaining`/`ExpectStatic` all build on) and carries a delete-when-upstream-ships note (testing-library/jest-dom#738 is open; 7.0.1 is latest). Delete it once that lands and `npm run build` still passes with it gone.
- The **85%** coverage gate (statements/branches/functions/lines), the `Format` (prettier-check) CI job, and the `Lint` (Oxlint) CI job are in AGENTS.md (Testing, Commands). Run `npx vitest run --coverage`, `npx prettier --check .`, and `npm run lint` before pushing.
