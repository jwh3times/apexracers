...
describe('api', () => {
  ...
  describe('401 retry interceptor', () => {
    ...
    // The per-verb retry cases that used to live here (GET/POST/PUT/DELETE/postForm/postJson,
    // each in a succeeds/throws pair) were ten copies of one branch reached through ten
    // different api methods. That branch is verb-independent, so it is now covered directly
    // against createHttpClient in http.test.ts. What remains here is refresh *mechanics* —
    // dedup, the callbacks, and token rotation — which still live in this module.
  });
